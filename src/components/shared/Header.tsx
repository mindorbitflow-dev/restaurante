  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
        ...
        {/* Left: Mobile Menu Toggle / Desktop Brand */}
        <div className="flex items-center gap-3">
          <button
            onClick={() => setMobileMenuOpen(!mobileMenuOpen)}
            className="md:hidden p-2 text-[#22C55E] hover:text-white transition-colors"
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            ...
            className="relative p-2 text-slate-300 hover:text-[#FBBF24] transition-colors duration-200"
      ...
      {mobileMenuOpen && (
        <div className="md:hidden fixed inset-0 top-[72px] bg-[#080E1E]/95 backdrop-blur-xl border-t border-slate-800 z-40 transition-all duration-300 animate-fade-in">
          <nav className="flex flex-col items-center justify-center h-full gap-8 p-4 pb-20">
            {navLinks.map((link) => (
              <Link
                ...
                href={link.href}
                onClick={(e) => link.isHash && handleNavClick(e, link.href)}
                className={`font-display text-sm tracking-widest uppercase py-2 font-bold ${
                  pathname === link.href ? 'text-[#FBBF24]' : 'text-slate-300 hover:text-white'
                }`}
              >
                {link.name}
            ...
            <Link
              ...
              className="flex items-center gap-2 px-6 py-3 rounded-full bg-[#FBBF24] text-black font-display text-xs uppercase tracking-widest font-bold hover:shadow-lg active:scale-95 transition-all mt-4"
            >
